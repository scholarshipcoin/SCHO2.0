...
void Shutdown()
{
    ...
    RenameThread("scholarship-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-loadblock=<file>", _("Imports blocks from external scho000??.dat file on startup"));
    ...
    strUsage += HelpMessageOpt("-reindex", _("Rebuild chain state and block index from the scho*.dat files on disk"));
#ifndef WIN32
...
void CleanupBlockRevFiles()
{
    ...
    // Glob all scho?????.dat and rev?????.dat files from the blocks directory.
    // Remove the rev files immediately and insert the scho file paths into an
    // ordered map keyed by block file index.
    LogPrintf("Removing unusable scho?????.dat and rev?????.dat files for -reindex with -prune\n");
    ...
    for (directory_iterator it(blocksdir); it != directory_iterator(); it++) {
        ...
        {
            if (it->path().filename().string().substr(0,3) == "scho")
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("scholarship-loadscho");
    CImportingNow imp;
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            if (!boost::filesystem::exists(GetBlockPosFilename(pos, "scho")))
                break; // No block files left to reindex
            FILE *file = OpenBlockFile(pos, true);
            if (!file)
                break; // This error is logged in OpenBlockFile
            LogPrintf("Reindexing block file scho%05u.dat...\n", (unsigned int)nFile);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Scholarship Core version %s\n", FormatFullVersion());
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Upgrading to 0.8; hard-link the old schonnnn.dat files into /blocks/
    ...
    if (!boost::filesystem::exists(blocksDir))
    {
        ...
        for (unsigned int i = 1; i < 10000; i++) {
            boost::filesystem::path source = GetDataDir() / strprintf("scho%04u.dat", i);
            if (!boost::filesystem::exists(source)) break;
            boost::filesystem::path dest = blocksDir / strprintf("scho%05u.dat", i-1);
            try {
                ...
                LogPrintf("Error hardlinking scho%04u.dat: %s\n", i, e.what());
                break;
            }
        }
        if (linked)
