...
const string strMessageMagic = "TrojanCoin Signed Message:\n";
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // TrojanCoin also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // TrojanCoin also requires low S in sigs
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (nSigOpsCount > MAX_BLOCK_SIGOPS)
            return state.DoS(100, error("ConnectBlock(): too many sigops"),
                             REJECT_INVALID, "bad-scho-sigops");
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            ...
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    LogPrintf("Pre-allocating up to position 0x%x in scho%05u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile);
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE)
        return state.DoS(100, false, REJECT_INVALID, "bad-scho-length", false, "size limits failed");
    ...
    if (nSigOps > MAX_BLOCK_SIGOPS)
        return state.DoS(100, false, REJECT_INVALID, "bad-scho-sigops", false, "out-of-bounds SigOpCount");
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL, bool fProofOfStake=true)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (mi == mapBlockIndex.end())
            return state.DoS(10, error("%s: prev block not found", __func__), 0, "bad-prevscho");
        pindexPrev = (*mi).second;
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)
            return state.DoS(100, error("%s: prev block invalid", __func__), REJECT_INVALID, "bad-prevscho");
...
void UnlinkPrunedFiles(std::set<int>& setFilesToPrune)
{
    for (set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        boost::filesystem::remove(GetBlockPosFilename(pos, "scho"));
        boost::filesystem::remove(GetBlockPosFilename(pos, "rev"));
        LogPrintf("Prune: %s deleted scho/rev (%05u)\n", __func__, *it);
    }
}
...
void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrint("prune", "Prune: target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d scho/rev pairs\n",
...
FILE* OpenBlockFile(const CDiskBlockPos &pos, bool fReadOnly) {
    return OpenDiskFile(pos, "scho", fReadOnly);
}
...
bool static LoadBlockIndexDB()
{
    ...
    // Check presence of scho files
    LogPrintf("Checking all scho files are present...\n");
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        CBufferedFile schodat(fileIn, 2*MAX_BLOCK_SIZE, MAX_BLOCK_SIZE+8, SER_DISK, CLIENT_VERSION);
        uint64_t nRewind = schodat.GetPos();
        while (!schodat.eof()) {
            boost::this_thread::interruption_point();

            schodat.SetPos(nRewind);
            nRewind++; // start one byte further next time, in case of failure
            schodat.SetLimit(); // remove former limit
            unsigned int nSize = 0;
            try {
                // locate a header
                unsigned char buf[MESSAGE_START_SIZE];
                schodat.FindByte(chainparams.MessageStart()[0]);
                nRewind = schodat.GetPos()+1;
                schodat >> FLATDATA(buf);
            ...
            try {
                // read block
                uint64_t nBlockPos = schodat.GetPos();
                if (dbp)
                    dbp->nPos = nBlockPos;
                schodat.SetLimit(nBlockPos + nSize);
                schodat.SetPos(nBlockPos);
                CBlock block;
                schodat >> block;
                nRewind = schodat.GetPos();
