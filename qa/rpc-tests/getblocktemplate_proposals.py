...
def template_to_bytearray(tmpl, txlist):
    schover = pack('<L', tmpl['version'])
    mrklroot = genmrklroot(list(dblsha(a) for a in txlist))
    timestamp = pack('<L', tmpl['curtime'])
    nonce = b'\0\0\0\0'
    scho = schover + a2b_hex(tmpl['previousblockhash'])[::-1] + mrklroot + timestamp + a2b_hex(tmpl['bits'])[::-1] + nonce
    scho += varlenEncode(len(txlist))
    for tx in txlist:
        scho += tx
    return bytearray(scho)
...
class GetBlockTemplateProposalTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_template(node, tmpl, txlist, 'inconclusive-not-best-prevscho')
