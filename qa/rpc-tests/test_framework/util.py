...
def wait_for_bitcoind_start(process, url, i):
    ...
    while True:
        if process.poll() is not None:
            raise Exception('scholarshipd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("BITCOIND", "scholarshipd"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            bitcoind_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: scholarshipd started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    if binary is None:
        binary = os.getenv("BLACKMORED", "scholarshipd")
    args = [ binary, "-datadir="+datadir, "-server", "-keypool=1", "-discover=0", "-rest", "-mocktime="+str(get_mocktime()) ]
    if extra_args is not None: args.extend(extra_args)
    bitcoind_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: scholarshipd started, waiting for RPC to come up")
