...
- [Development guidelines](#development-guidelines)
    - [General Scholarship Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Scholarship Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Scholarship Core is a multi-threaded application, and deadlocks or other
...
Scholarship Core can be compiled with various "sanitizers" enabled, which add
...
- [ThreadImport (`b-loadscho`)](https://doxygen.bitcoincore.org/init_8cpp.html#ae9e290a0e829ec0198518de2eda579d1)
  : Loads blocks from `scho*.dat` files or `-loadblock=<file>` on startup.
...
However, in open source software such as Scholarship Core, where everyone uses
...
pay attention to for reviewers of Scholarship Core code.

General Scholarship Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Scholarship Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Scholarship Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Scholarship Core so that they can be integrated
...
file descriptors, it will cause problems with Scholarship's `select()` loop, because
...
compatibility between nodes. This happened in Scholarship 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Scholarship's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Scholarship Core, the wallet was always
