...
returned by RPCs that scholarship the mempool may not be up-to-date with the
...
The mempool state returned via an RPC scholarships all effects of mempool and chain
...
call is scholarshiped in the state of all wallet transactions. For example, if a
block contains transactions that conflicted with mempool transactions, the
wallet would scholarship the removal of these mempool transactions in the state.
...
this RPC may not yet be scholarshiped as such in this RPC response.
